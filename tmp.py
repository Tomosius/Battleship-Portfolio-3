    ...
    # Modify ship details
        ...
        try:
            ...
            if isinstance(new_size, int) and isinstance(new_qty, int):
                ...
                tmp_fleet = copy.deepcopy(fleet)
                tmp_map = create_map(height, width, symbol)
                tmp_fleet[ship_name]["Size"] = new_size
                tmp_fleet[ship_name]["Quantity"] = new_qty
                check_result = game_adjust_check_if_fleet_fits_on_map(tmp_map, tmp_fleet)
                if check_result == False:
                    print(" ships do not fit on map, change parameters")
                    return False
                else:
                    print("check result is true")
                    fleet[ship_name]["Size"] = new_size
                    fleet[ship_name]["Quantity"] = new_qty
                    return fleet
        except ValueError:
...
def cpu_vs_cpu():
    ...
    new_fleeet = copy.deepcopy(DEFAULT_FLEET)


    # Deploy all of CPU's ships (assuming the function 'cpu_deploy_all_ships' exists)
    map_cpu_display , fleet_cpu = cpu_deploy_all_ships(new_map_display, new_fleeet)
